...
export function getAllModelUrls() {
  ...
  log(`[ModelPreloadConfig] 生成了 ${urls.length} 个模型URL`);
  return urls;
}
...
export function getHighPriorityModelUrls() {
  ...
  log(`[ModelPreloadConfig] 生成了 ${urls.length} 个高优先级模型URL`);
  return urls;
}
...
export function getMediumPriorityModelUrls() {
  ...
  log(`[ModelPreloadConfig] 生成了 ${urls.length} 个中等优先级模型URL`);
  return urls;
}
...
export function getLowPriorityModelUrls() {
  ...
  log(`[ModelPreloadConfig] 生成了 ${urls.length} 个低优先级模型URL`);
  return urls;
}
...
export function getModelUrlsByPriority(priorities = ['high', 'medium']) {
  ...
  log(
    `[ModelPreloadConfig] 根据优先级 ${priorities.join(', ')} 生成了 ${
      uniqueUrls.length
    } 个唯一模型URL`
  );
